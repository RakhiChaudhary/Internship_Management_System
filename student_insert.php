...
$cname1Err = $cname2Err = $cname3Err = $selfErr = $sidErr = $uploadErr = NULL;
$flag=true;
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
      if (empty($_POST["sid"])) {
       ...
       empty($_POST["sid"])==false){  
        $sidErr = "Roll no can be of length 7";
        $flag = false;
      }
      else{
        $sid = $_POST['sid'];
    ...
    empty($_POST["company1"])==false) {
     ...
     if (preg_match('/^[-a-zA-Z0-9 ]+$/', $_POST["company2"])==false &&
      ...
      empty($_POST["self"])==false) {
        $selfErr = "Self can be alphabet";
        $flag = false;
      }
      else if(strlen($_POST["self"])>200){
        $selfErr  = "Self can be of maximum length 200";
        $flag = false;
      } else {
          $self = $_POST["self"];
      }
    ...
    if($flag){
      ...
      if($result){
         ...
         echo '<div class="alert alert-success">Thank You! Data is inserted </div>';
          header("Refresh: 3; URL=homepagetpo.php");
      }
      else{
  ...
  <?php include 'navbar1.php';?>
    ...
    <div class="wrapper">
      <div class="container_form">
        ...
        <div class="mb-3">
            <label for="" style="padding-bottom:15px;">Upload Resume</label><br>
